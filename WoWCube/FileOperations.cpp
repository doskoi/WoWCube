...
bool CFileOperation::CanDelete(CString sPathName)
{
	...
	if (dwAttr & FILE_ATTRIBUTE_READONLY)
	{
		if (m_bAskIfReadOnly)
		{
			...
			CString sText = sTmp + CString(_T("是只读文件，确认要删除吗？"));
			int iRes = MessageBox(NULL, sText, _T("警告"), MB_YESNOCANCEL | MB_ICONQUESTION);
			switch (iRes)
			{
				case IDYES:
				{
