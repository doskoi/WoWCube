--[[
	...
	Revision: $Id: CoreScan.lua 3985 2009-01-15 12:04:47Z norganna $
...
Commitfunction = function()
	...
	for pos, data in ipairs(scandata.image) do
		...
		if GetTime() - lastPause >= processingTime then
			lib.ProgressBars(CommitProgressBar, 100*i/totali, true, "≈ƒ¬Ù÷˙ ÷: ¥¶¿ÌΩ◊∂Œ 1")
			coroutine.yield()
			lastPause = GetTime()
		end
		if link then
	...
	for index, data in ipairs(TempcurScan) do
		i = i + 3
		if GetTime() - lastPause >= processingTime then
			lib.ProgressBars(CommitProgressBar, 100*i/totali, true, "≈ƒ¬Ù÷˙ ÷: ¥¶¿ÌΩ◊∂Œ 2")
...
-- end of debugging code
	...
	for pos = #scandata.image, 1, -1 do
		...
		i = i + 1
		if GetTime() - lastPause >= processingTime then
			lib.ProgressBars(CommitProgressBar, 100*i/totali, true, "≈ƒ¬Ù÷˙ ÷: ¥¶¿ÌΩ◊∂Œ 3")
			coroutine.yield()
			lastPause = GetTime()
		end
...
AucAdvanced.RegisterRevision("$URL: http://svn.norganna.org/auctioneer/branches/5.2/Auc-Advanced/CoreScan.lua $", "$Rev: 3985 $")
