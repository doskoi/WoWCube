--[[
	...
	Revision: $Id: Auc-Util-AutoMagic.lua 4005 2009-01-19 21:31:47Z anaral $
	...
	if not (get("util.automagic.depositTT")) then
		if hyperlink then
			...
			if (ttdepcost == nil) then
				tooltip:AddLine("|cff336699 未知保证金 |r")
			elseif (ttdepcost == 0) then
				tooltip:AddLine("|cff336699 无需保证金 |r")
			else
				tooltip:AddLine("|cffCCFF99"..get("util.automagic.deplength").."小时 保证金 : |r" , ttdepcost)
			end
		end
	end
end
...
lib.makeautosellgui()
AucAdvanced.RegisterRevision("$URL: http://svn.norganna.org/auctioneer/branches/5.2/Auc-Util-AutoMagic/Auc-Util-AutoMagic.lua $", "$Rev: 4005 $")
