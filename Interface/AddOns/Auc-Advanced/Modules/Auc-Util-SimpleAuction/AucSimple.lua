...
function lib.ProcessTooltip(tooltip, name, link, quality, quantity, cost, additional)
	...
	local coinsBid, coinsBuy, coinsBidEa, coinsBuyEa = "无","无","无","无"
	if market > 0 then
	...
	if quantity == 1 then
		local text = string.format("拍卖助手: %s 竞标价/%s 一口价", coinsBid, coinsBuy)
		tooltip:AddLine(text)
	else
		local text = string.format("拍卖助手 x%d: %s 竞标价/%s 一口价", quantity, coinsBid, coinsBuy)
		local textea =  string.format("%s(或单独: %s/%s)", whitespace(5), coinsBidEa, coinsBuyEa)
		tooltip:AddLine(text)
	...
	if fixbid then
		...
		if quantity == 1 then
			local text = string.format("%sFixed: %s 竞标价/%s 一口价", whitespace(12), coinsBid, coinsBuy)
			tooltip:AddLine(text)
		else
			local text = string.format("%sFixed x%d: %s 竞标价/%s 一口价", whitespace(12), quantity, coinsBid, coinsBuy)
			tooltip:AddLine(text)
		end
	end
	if get("util.simpleauc.tooltip.undercut") then
		if lowBid and lowBid > 0 then
			...
			if quantity == 1 then
				local text = string.format("%s削弱: %s 竞标价/%s 一口价", whitespace(8), coinsBid, coinsBuy)
				tooltip:AddLine(text)
			else
				local text = string.format("%s削弱 x%d: %s 竞标价/%s 一口价", whitespace(8), quantity, coinsBid, coinsBuy)
				tooltip:AddLine(text)
			end
		else
			tooltip:AddLine("  没有竞争")
		end
	end
end
...
AucAdvanced.RegisterRevision("$URL: http://svn.norganna.org/auctioneer/branches/5.2/Auc-Util-SimpleAuction/AucSimple.lua $", "$Rev$")
