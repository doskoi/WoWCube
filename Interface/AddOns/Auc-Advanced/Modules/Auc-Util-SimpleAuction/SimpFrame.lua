...
function private.UpdateDisplay()
	...
	if cBid > 0 then
		...
		coinsBid = "no"
		coinsBidEa = "no"
		frame.err:SetText("错误: 没有设置出价")
	end
	if cBuy > 0 then
		coinsBuy = coins(cBuy)
		coinsBuyEa = coins(cBuy/cStack)
		if cBuy < cBid then
			frame.err:SetText("错误: 一口价不能小于竞标价")
		end
	else
		coinsBuy = "no"
		coinsBuyEa = "no"
	...
	local text
	if (cStack > 1) then
		text = string.format("拍卖 %d %s 堆 %d 个每叠的价格在 %s 竞标价/%s 一口价/每堆 (%s/%s 每个)", cNum, lots, cStack, coinsBid, coinsBuy, coinsBidEa, coinsBuyEa)
	else
		text = string.format("拍卖 %d %s 个此物品价格在 %s 竞标价/%s 一口价/每个", cNum, lots, coinsBid, coinsBuy)
	end
...
function private.LoadItemLink(itemLink, size)
	...
	if itemLink then
		...
		frame.name:SetText("拖动物品至空格")
	end
	frame.info:SetText("从你包裹中拖动物品至此用于拍卖")
	frame.fees:SetText("")
	frame.err:SetText("-- 没有选择物品 --")
...
function private.CreateFrames()
	...
	frame.title:SetText("拍卖助手 - 简易拍卖发布界面")
	...
	frame.minprice.label:SetText("竞标价:")
	...
	frame.buyout.label:SetText("一口价:")
	...
	frame.duration.label:SetText("时间:");
	...
	frame.stacks.label:SetText("堆: (堆数 x 体积)");
	...
	frame.options.label:SetText("选项:")
	...
	frame.options:AddOption("matchmy", "匹配当前价格")
	frame.options:AddOption("undercut", "略低于竞争对手")
	frame.options:AddOption("remember", "记住修改过的价格")
	...
	frame.imageview.sheet = ScrollSheet:Create(frame.imageview, {
		{ "卖家", "TEXT", get("util.simpleauc.columnwidth.Seller")}, --89
		{ "剩余",   "INT",  get("util.simpleauc.columnwidth.Left")}, --32
		{ "堆",    "INT",  get("util.simpleauc.columnwidth.Stk")}, --32
		{ "最低/个", "COIN", get("util.simpleauc.columnwidth.Min/ea"), { DESCENDING=true } }, --65
		{ "当前/个", "COIN", get("util.simpleauc.columnwidth.Cur/ea"), { DESCENDING=true } }, --65
		{ "一口/个", "COIN", get("util.simpleauc.columnwidth.Buy/ea"), { DESCENDING=true, DEFAULT=true } }, --65
		{ "竞标价", "COIN", get("util.simpleauc.columnwidth.MinBid"), { DESCENDING=true } }, --76
		{ "当前价", "COIN", get("util.simpleauc.columnwidth.CurBid"), { DESCENDING=true } }, --76
		{ "一口价", "COIN", get("util.simpleauc.columnwidth.Buyout"), { DESCENDING=true } }, --80
