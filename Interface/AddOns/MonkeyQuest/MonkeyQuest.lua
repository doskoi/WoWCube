...
MonkeyQuest.m_iNumQuestButtons = 50;		-- 40[50] is the max possible entries in the quest log (20[25] quests and 20[25] different locations) [Patch 2.0]
...
function MonkeyQuest_OnLoad()
	...
	if ( GetLocale() == "zhCN") then
		MonkeyQuestTitleText:SetFont("ZYHei.ttf",11,"OUTLINE, MONOCHROME");
	elseif ( GetLocale() == "zhTW" ) then
		MonkeyQuestTitleText:SetFont("bHEI00M.TTF",11,"OUTLINE, MONOCHROME");
	else
		MonkeyQuestTitleText:SetFont("FRIZQT__.TTF",11,"OUTLINE, MONOCHROME");
	end
...
--	no longer necessary, using YourSetBagItem now ( since Patch 2.2.0 )
...
function MonkeyQuest_OnEvent(event)
    ...
    if (event == "ADDON_LOADED" and arg1 == "MonkeyQuest" ) then
								...
								for ii = 1, GetNumQuestLeaderBoards(), 1 do
									...
									if (objectiveType == "item" or objectiveType == "monster" or objectiveType == "object") then
										...
										if (objectiveName ~= nil and objectiveName ~= "  slain" and objectiveName ~= " ") then
											...
											--	PlaySoundFile(MONKEYQUEST_SOUND_COMPLETE);
											end
