...
SFWM_VERSION = "v3.34.30000";

SFWM = {};

local ShowMapOverlayInfo = {
...
function Show_GetMainTable(mapFileName)
	local t = ShowMapOverlayInfo[mapFileName]
	if not t then t = { }; ShowMapOverlayInfo[mapFileName] = t end
	return t
end
...
local function getErrataTable(mapFileName)
	local t = ShowFullWorldMap.Errata[mapFileName]
	if not t then t = { }; ShowFullWorldMap.Errata[mapFileName] = t end
	return t
end
...
function worldMapFrame_UpdateOverlays(dtlFrame, ovrLay, scalingFactor, oAlpha)
	...
	local i, tname, oinfo
	local discovered = {}
	local numOverlays = Show_GetNumMapOverlays()
--	local numOverlays = GetNumMapOverlays();
	...
	local zoneTable = Show_GetMainTable(mapFileName)
	...
	for i,oinfo in next,zoneTable do
		...
		if ( ShowFullWorldMap.Enabled or discovered[textureName] ) then
				...
				for k=1, numTexturesWide do
					...
					if ( texture ) then
						...
						elseif ( ( clearRidge ) and ( string.find(textureName, "RidgeofMadness") ) ) then
							...
							if(ShowFullWorldMap.colorStyle == 0) then
								texture:SetVertexColor(0.2,0.6,1.0) -- EMERALD

							elseif(ShowFullWorldMap.colorStyle == 1) then
								texture:SetVertexColor(1.0,1.0,1.0)

							elseif ( ( ShowFullWorldMap.colorStyle == 2 ) and ( ShowFullWorldMap.colorArray ) ) then
								local col = ShowFullWorldMap.colorArray;
								texture:SetVertexColor(col.r, col.g, col.b);

							else
								message("Corrupt color selection detected, resetting to blue.")
								texture:SetVertexColor(0.2,0.6,1.0) -- EMERALD
								ShowFullWorldMap.colorStyle = 0
							end

							if ( oAlpha ) then
								texture:SetAlpha(oAlpha);

							elseif(ShowFullWorldMap.transparency >= 0.0 and ShowFullWorldMap.transparency <= 1.0) then
								texture:SetAlpha(ShowFullWorldMap.transparency)
	
							else
								message("Corrupt transparency value detected, resetting to opaque.")
								ShowFullWorldMap.transparency = 1.0
								texture:SetAlpha(ShowFullWorldMap.transparency)
							end
...
Show_GetNumMapOverlays = GetNumMapOverlays;
function GetNumMapOverlays()
	if(NUM_WORLDMAP_OVERLAYS == 0) then return Show_GetNumMapOverlays(); end
	return 0;
end
...
local Show_BattlefieldMinimap_Update;
...
function ShowFullWorldMap_Command(msg)
	local cmd = string.lower(msg)
	if(msg == "blue") then
		ShowFullWorldMap.colorStyle = 0;
		Print(SFWM_COLORME_BLUE);

	elseif(msg == "normal") then
		ShowFullWorldMap.colorStyle = 1;
		Print(SFWM_COLORME_NOT);
	...
	elseif(string.find(cmd, "^trans [01]%.[0-9]+$")) then
		...
		if(trans >= 0.0 and trans <= 1.0) then
			ShowFullWorldMap.transparency = trans;
			Print(SFWM_TRANSME_DONE..trans);
		else
			Print(SFWM_TRANSME_ERROR);
		end

	elseif (msg == "rainbow") then
		SFWM_SetUpColourPicker();
...
function ShowFullWorldMapFrame_OnLoad()
	this:RegisterEvent("VARIABLES_LOADED");
	this:RegisterEvent("ADDON_LOADED");
	SLASH_SFWM1 = "/showfullworldmap";
	SLASH_SFWM2 = "/sfwm"; -- A shortcut or alias
	SlashCmdList["SFWM"] = ShowFullWorldMap_Command;
end

function ShowFullWorldMapFrame_OnUpdate(self, elapsed)
	if ( SFWM.loadCheck ) then
		SFWM.loadCheck = SFWM.loadCheck + elapsed;
		if ( SFWM.loadCheck > 1 ) then
	...
	elseif ( ( SFWM.loaded ) and ( SFWM.loadBFM ) ) then
		SetBFMHooks();
...
function ShowFullWorldMapFrame_OnEvent()
	if (event == "VARIABLES_LOADED") then

		if(not ShowFullWorldMap.colorStyle) then ShowFullWorldMap.colorStyle = 0 end
		if(not ShowFullWorldMap.transparency) then ShowFullWorldMap.transparency = 1.0 end
		...
		if ( not ShowFullWorldMap.version ) then
			...
			ShowFullWorldMap.Errata = {};
		end
		ShowFullWorldMap.version = SFWM_VERSION;

		local mapFileName, errataTable, i, oinfo
		for mapFileName,errataTable in next,ShowFullWorldMap.Errata do
			-- hack to keep strings short and maintainable.
			local prefix = "Interface\\WorldMap\\"..mapFileName.."\\"

			-- build a temporary index of the mainTable, from textureName to index!
			local mainIndex = {}
			local mainTable = Show_GetMainTable(mapFileName)
		...
		if ( ShowFullWorldMap.Enabled == nil ) then
			ShowFullWorldMap.Enabled = true;
		end

		SFWM.loadCheck = 0.001;
		SFWM.loaded = true;

		--DEFAULT_CHAT_FRAME:AddMessage("SFWM (Fan's Update) |c0000FF00"..SFWM_VERSION.."|r", 0.64, 0.21, 0.93);
...
function SFWM_KeyBind_Toggle()
	if ( ShowFullWorldMap.Enabled == true ) then
		ShowFullWorldMap.Enabled = false;
		ShowWorldMapShowAllCheckButton:SetChecked(0);
	else
		ShowFullWorldMap.Enabled = true;
		ShowWorldMapShowAllCheckButton:SetChecked(1);
	end
...
function SFWM_SetUpColourPicker()
	local col;

	ColorPickerFrame.hasOpacity = true;
	ColorPickerFrame.opacity = 1 - ShowFullWorldMap.transparency;
	ColorPickerFrame.func = SFWM_AcceptColour;
	ColorPickerFrame.cancelFunc = SFWM_CancelColourPicker;
	if ( ShowFullWorldMap.colorArray ) then
		col = ShowFullWorldMap.colorArray;
	else
...
function SFWM_AcceptColour()
	ShowFullWorldMap.transparency = 1 - OpacitySliderFrame:GetValue();
	local r, g, b = ColorPickerFrame:GetColorRGB();
	ShowFullWorldMap.colorArray = {};
	ShowFullWorldMap.colorArray.r = r;
	ShowFullWorldMap.colorArray.g = g;
	ShowFullWorldMap.colorArray.b = b;
	ShowFullWorldMap.colorStyle = 2;
end

function SFWM_CancelColourPicker(col)
	ShowFullWorldMap.transparency = 1 - ColorPickerFrame.opacity;
	if ( ( col[1] == 0.2 ) and ( col[2] == 0.6 ) and ( col[3] == 1.0 ) ) then
		ShowFullWorldMap.colorArray = nil;
		ShowFullWorldMap.colorStyle = 0;
	else
		ShowFullWorldMap.colorArray = {};
		ShowFullWorldMap.colorArray.r = r;
