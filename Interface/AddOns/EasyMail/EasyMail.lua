...
function EasyMail_OnEvent(self, event, ...)
	...
	if ( event == "PLAYER_LOGIN" ) then
		...
		if (not EasyMail_SavedVars[CurRealm][CurFaction].MailListLen) then
			-- Total number of addressees to show in list
		...
		if (not EasyMail_SavedVars.AutoAdd) then
			EasyMail_SavedVars.AutoAdd = "N";
		end
		if (not EasyMail_SavedVars.Guild) then
			EasyMail_SavedVars.Guild = "N";
		end
		if (not EasyMail_SavedVars.Friends) then
			EasyMail_SavedVars.Friends = "N";
		end
		if (not EasyMail_SavedVars.ClickGet) then
			EasyMail_SavedVars.ClickGet = "N";
		end
		if (not EasyMail_SavedVars.ClickDel) then
			EasyMail_SavedVars.ClickDel = "N";
		end
		if (not EasyMail_SavedVars.DelPending) then
			EasyMail_SavedVars.DelPending = "N";
		end
		if (not EasyMail_SavedVars.Money) then
			EasyMail_SavedVars.Money = "Y";
		end
		if (not EasyMail_SavedVars.Total) then
			EasyMail_SavedVars.Total = "Y";
		end
		if (not EasyMail_SavedVars.TextTooltip) then
			EasyMail_SavedVars.TextTooltip = "N";
		end
		if (not EasyMail_SavedVars.DelPrompt) then
		...
		hooksecurefunc("SendMailFrame_Reset", function() 
				if (EasyMail_Enabled) then 
					EasyMail_SetAddressee(nil, CurrentAddressee); 
				end 
			end);
		...
		-- Define slash commands and other options
	...
	if ( event == "MAIL_SHOW" and EasyMail_Enabled) then
	...
	-- odd times)
	if ( event == "MAIL_SEND_SUCCESS" and SendingMail and EasyMail_Enabled) then
	...
	-- Process guild list
	if (EasyMail_Enabled and IsInGuild() and (event == "GUILD_ROSTER_UPDATE" or event == "PLAYER_GUILD_UPDATE") 
			and EasyMail_SavedVars.Guild == "Y") then
		EasyMail_GetGuildNames();
	end
	
	-- Process friend list
	if (EasyMail_Enabled and (event == "FRIENDLIST_SHOW" or event == "FRIENDLIST_UPDATE") and EasyMail_SavedVars.Friends == "Y") then
		EasyMail_GetFriends();
	end
end
...
function EasyMail_GetAddressee()
	...
	CurrentAddressee = strupper(strsub(text, 1, 1))..strlower(strsub(text, 2, strlen(text)));
end
...
function EasyMail_SaveAddressee(namestr, addlogged)
	...
	local namestr = string.upper(string.sub(namestr,1,1))..string.lower(string.sub(namestr,2));
...
function EasyMail_MailItemOnClickHandler(self, button)
	...
	if (not EasyMail_Enabled or button ~= "RightButton") then
...
function EasyMail_ShowCheckBoxes()
	if (EasyMail_Enabled) then
		...
		MailItem1:SetPoint("TOPLEFT", "InboxFrame", "TOPLEFT", MailItemX, MailItemY);

		for t = 1, INBOXITEMS_TO_DISPLAY do
			getglobal("MailItem"..t):SetWidth(MailItemWidth);
			getglobal("MailItem"..t.."Subject"):SetWidth(SubjectWidth);
			getglobal("MailItem"..t.."ExpireTime"):SetPoint("TOPRIGHT", "MailItem"..t, "TOPRIGHT", ExpireTimeX, ExpireTimeY);
			getglobal("EasyMail_CheckButton"..t):Hide();
		end
	end
end
...
function EasyMail_MoneySubject()
	if (EasyMail_Enabled and (MoneySubject or SendMailSubjectEditBox:GetText() == "")) then
...
function GetShortMoneyString(money)
	...
	local copperString = "";
	if ( copper > 0 ) then
		copperString = copper..strlower(strsub(EasyMail_Copper, 1, 1));
	end
	
	local silverString = "";
	if ( silver > 0 ) then
		silverString = silver..strlower(strsub(EasyMail_Silver, 1, 1));
		if ( copper > 0 ) then
	...
	if ( tonumber(gold) > 0 ) then
		goldString = gold..strlower(strsub(EasyMail_Gold, 1, 1));
		if ( copper > 0 or silver > 0 ) then
...
function EasyMail_Tooltip(self)
   if (EasyMail_SavedVars.TextTooltip ~= "Y" or not EasyMail_Enabled) then
      return;
   end
...
---------------------------------------------------
-- EasyMail_SendeeAutocomplete
-- Perform auto-complete with names on recently-mailed list
---------------------------------------------------
function EasyMail_SendeeAutocomplete(self)
	local text = self:GetText();
	local textlen = strlen(text);

	for i=1, #SortedList do
		if (text and (strfind(strupper(SortedList[i]), strupper(text), 1, 1) == 1) ) then
	...
	Old_PlayerNameAutocomplete(self);
end
